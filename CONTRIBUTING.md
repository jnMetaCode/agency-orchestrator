...
- **给交付步写 `deliverables` + `acceptance`**（下面这段是踩过坑总结的配方）

#### 交付物与验收怎么写（照着做，别凭感觉）

盲评实测：同一模板、同一模型档位，只加了这两样，多智能体从「8.5 : 8.5 打平（低可信）」变成
「9.0 : 7.0 胜（高可信）」——差距不是来自多几个角色，而是**把验收写成了数据**，引擎替你执行。

1. **`deliverables: [<交付步 id>]`**：声明哪几步是要拿走的成品。导出 / `--compare` 盲评 /
   summary 的 ⭐ 都只认它，中间的大纲、人设、审读意见留在运行目录里不碍事。
2. **`acceptance` 只写「能从产出文本本身核对」的条目**。最稳的写法是**照抄你在 `task` 里已经
   要求的输出结构**：几个小节、每节几条、表格几列、要不要标时间/负责人。
   - ✅ 「四个小节齐全：A / B / C / D」「话题标签正好 10 个且以 # 开头」「行动项每条写明负责人与时间」
   - ❌ 「每段不超过 5 行」（取决于渲染宽度）、「语言优美」「逻辑清晰」（没法客观核对）
3. **结构交给 `assert`，内容交给 `acceptance`**：文件块个数、字数下限这类「数数就能判」的，用
   `assert`（不过模型、不花 token）。少一个文件模型验收员看不出来，而 `--materialize` 会当场落空。
4. **提交前真跑一次，确认「第一次就过、没触发返工」**：
   `ao run workflows/你的模板.yaml -i ...`，看那步是不是 `验收 ✓` 而不是 `验收 ✓（返工 1 轮后通过：…）`。
   **一条总是触发返工的 acceptance 是负资产**——它让每个用户的每次运行都多付一次模型调用。
   返工过的话，`metadata.json` 的 `verification.firstFailed` 会告诉你是哪一条逼的，照着改或删掉。
