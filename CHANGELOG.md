...
- **旗舰模板补上 `deliverables` + 交付步的 `acceptance`，并附前后实测**。`compose` 的提示词一直教用户「至少给
  ...
  （小红书爆款笔记 / 抖音脚本 / 会议纪要——它们的 task 里本来就写死了输出结构，验收条目照着结构写即可）补齐：**「每个声明的交付物都写了 acceptance」的模板从 4 个涨到 21 个**（全库 70 个）。后面几批挑的都是 **task 里已经写死了小节结构**的模板（安全审计 / 合同审查 / OKR 拆解 / PR 评审 / 竞品分析 / 面试题库），验收条目照抄结构即可——这九个真跑下来全部一次通过、零返工；唯一踩雷的是主观条目（给 ai-opinion 写的「每段不超过 5 行」，依赖渲染宽度，真多跑了一轮），且**不再有「声明了交付物却一条验收都没写」的模板**（最后一个是 `一人公司-方案到代码`：出代码那步除 acceptance 外还加了 `assert.emits_files: 3`——少一个文件模型验收员看不出来，而 `--materialize` 会当场落空）。**每份 `acceptance`
