...
- **旗舰模板补上 `deliverables` + 交付步的 `acceptance`，并附前后实测**。`compose` 的提示词一直教用户「至少给
  ...
  （小红书爆款笔记 / 抖音脚本 / 会议纪要——它们的 task 里本来就写死了输出结构，验收条目照着结构写即可）补齐：**「每个声明的交付物都写了 acceptance」的模板从 4 个涨到 18 个**（全库 70 个；后一批是安全审计 / 合同审查 / OKR 拆解，挑的都是 task 里写死了小节结构、验收条目可照抄结构的），且**不再有「声明了交付物却一条验收都没写」的模板**（最后一个是 `一人公司-方案到代码`：出代码那步除 acceptance 外还加了 `assert.emits_files: 3`——少一个文件模型验收员看不出来，而 `--materialize` 会当场落空）。**每份 `acceptance`
