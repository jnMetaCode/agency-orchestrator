...
将 `ao-workflow-runner.md` 复制到 `.kiro/steering/` 后，Kiro 会自动加载该 steering 文件。在 Kiro 中直接说：
...
|------|:----------:|:--------:|
| 需要 API key | 不需要 | 需要 |
| 需要安装 | ao-workflow-runner.md → .kiro/steering/ | agency-orchestrator |
| 执行环境 | Kiro 会话内 | 终端 |
...
```bash
ao roles
# 或在 Kiro 中说：列出 agency-agents-zh 里的所有角色
```
