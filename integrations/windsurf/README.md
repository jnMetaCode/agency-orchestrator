...
cp .ao-tmp/integrations/windsurf/.windsurfrules .windsurfrules
rm -rf .ao-tmp

# 3. 开始使用
# 在 Windsurf 中直接说：运行 workflows/story-creation.yaml
```

如果项目根目录已有 `.windsurfrules`，可将内容追加而非覆盖：

```bash
# 替换上面第 2 步中的 cp 命令为：
cat .ao-tmp/integrations/windsurf/.windsurfrules >> .windsurfrules
```
...
将 `.windsurfrules` 复制到项目根目录后，Windsurf Cascade 会自动加载该规则文件。在 Cascade 中直接说：
...
| 特性 | Cascade 模式 | CLI 模式 |
|------|:----------:|:--------:|
| 需要 API key | 不需要 | 需要 |
| 需要安装 | .windsurfrules → 项目根目录 | agency-orchestrator |
| 执行环境 | Windsurf Cascade 会话内 | 终端 |
...
```bash
ao roles
# 或在 Windsurf 中说：列出 agency-agents-zh 里的所有角色
```
