...
**One sentence → full plan · 276 expert AI roles · Zero-code YAML · 12 LLM providers · key supported (DeepSeek recommended), plus 8 key-free options**
...
| API key | — | Required | **Key supported; 8 key-free options too** |
...
llm:
  provider: "deepseek"          # No API key: claude-code / gemini-cli / copilot-cli / codex-cli / hermes-cli / codebuddy-cli / ollama
  model: "deepseek-chat"
...
## 12 LLM Providers — 8 Need No API Key
...
| `llm.provider` | string | Yes | `claude-code` / `gemini-cli` / `copilot-cli` / `codex-cli` / `openclaw-cli` / `hermes-cli` / `codebuddy-cli` / `ollama` / `claude` / `deepseek` / `openai` |
