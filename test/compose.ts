...
await test('降档表的键都是引擎真认识的 provider（写错=永远静默 no-op）', async () => {
  ...
  const known = new Set([...API_PROVIDERS.map((x) => x.id), 'claude', 'claude-code', 'antigravity-cli', 'gemini-cli', 'copilot-cli', 'codex-cli', 'openclaw-cli', 'hermes-cli', 'codebuddy-cli', 'ollama']);
