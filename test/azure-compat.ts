...
// 用容器而不是可空变量：TS 的控制流分析看不到回调里的赋值，后面的 captured 会被收窄成 never
const cap: { last: { body: any; headers: any } | null } = { last: null };
const srv = http.createServer((req, res) => {
  let b = '';
  req.on('data', (d) => (b += d));
  req.on('end', () => {
    cap.last = { body: JSON.parse(b), headers: req.headers };
...
assert(cap.last !== null && 'max_completion_tokens' in cap.last!.body, 'Azure 用 max_completion_tokens');
assert(cap.last !== null && !('max_tokens' in cap.last!.body), 'Azure 不再发 max_tokens');
assert(cap.last !== null && cap.last!.headers['api-key'] === 'k', 'Azure 带 api-key header');
...
assert(cap.last !== null && 'max_tokens' in cap.last!.body, '普通端点仍用 max_tokens');
assert(cap.last !== null && cap.last!.headers['api-key'] === undefined, '普通端点不带 api-key header');
...
assert(cap.last !== null && 'max_completion_tokens' in cap.last!.body, 'AO_OPENAI_TOKENS_PARAM 覆盖生效');
...
assert(cap.last !== null && cap.last!.body.reasoning_effort === 'high', 'params.reasoning_effort 透传进请求体 (#90)');
assert(cap.last !== null && cap.last!.body.top_p === 0.9, 'params.top_p 透传进请求体');
assert(cap.last !== null && cap.last!.body.stream === true, 'params 不能覆盖 stream（流式解析保护）');
assert(cap.last !== null && cap.last!.body.model === 'deepseek-reasoner', 'params 不能覆盖 model');
...
assert(cap.last !== null && 'max_completion_tokens' in cap.last!.body, '#99 o系列模型(o1-mini)非 Azure 也自动用 max_completion_tokens');
await rm.chat('s', 'u', { provider: 'openai', model: 'gpt-5', max_tokens: 100 });
assert(cap.last !== null && 'max_completion_tokens' in cap.last!.body, '#99 gpt-5 自动用 max_completion_tokens');
await rm.chat('s', 'u', { provider: 'openai', model: 'gpt-4o', max_tokens: 100 });
assert(cap.last !== null && 'max_tokens' in cap.last!.body && !('max_completion_tokens' in cap.last!.body), '#99 gpt-4o 不误判（仍 max_tokens）');

// 未显式指定 max_tokens 时的默认上限：推理模型放大到 32768，普通仍 4096
await rm.chat('s', 'u', { provider: 'openai', model: 'o1' });
assert(cap.last !== null && cap.last!.body.max_completion_tokens === 32768, '#99 推理模型默认上限放大到 32768');
await rm.chat('s', 'u', { provider: 'openai', model: 'gpt-4o' });
assert(cap.last !== null && cap.last!.body.max_tokens === 4096, '#99 普通模型默认上限仍 4096');
...
assert(cap.last !== null && cap.last!.body.max_completion_tokens === 4096, '#99 非推理 Azure 部署默认上限仍 4096(不放大,防过大 400)');
...
const srv2 = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    cap.last = { body, headers: req.headers };
    res.writeHead(200, { 'Content-Type': 'text/event-stream' });
    res.write(`data: ${JSON.stringify({ choices: [{ delta: { content: 'ok' }, finish_reason: 'stop' }] })}\n\n`);
    res.write('data: [DONE]\n\n');
    res.end();
...
assert(cap.last !== null && 'max_completion_tokens' in cap.last!.body && !('max_tokens' in cap.last!.body), '#99 重试改用 max_completion_tokens');
...
assert(cap.last !== null && 'max_tokens' in cap.last!.body && !('max_completion_tokens' in cap.last!.body), '#99 反向：重试改回 max_tokens');
