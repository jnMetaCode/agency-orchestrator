...
> 🆕 **可分享报告页**：跑出满意成果后，运行详情一键「分享页」（CLI 为 `ao report`）——专家分工时间线 + 每步产出的自包含单文件 HTML，直接发群里，对方无需装 AO。
> 🆕 **结果群推送**：`ao run --notify <webhook>` 跑完自动推送到钉钉 / 飞书 / 企业微信群（按域名自动适配机器人格式），配合 cron 就是"AI 团队每天定点交活"。
> 🆕 **社区模板**：工作流页新增「社区模板」分区——收录制远程清单，一键导入（保存前经引擎校验），你的好工作流也可以投稿给所有用户用。
