        ...
        <div className="container-page pb-20">
          ...
          <p className="mt-1.5 text-sm text-muted-foreground">
            {media === "image"
              ...
              : (lang === "en"
                ? "Text-to-video templates: pick a genre, fill a few variables, get a full 5-part cinematic prompt. Run them with a `type: video` step in a workflow. Every sample clip below was actually generated by Agency Orchestrator (provider / model shown on each card)."
                : "文生视频模板：挑题材、填几个变量，拿到完整的 5 段式电影感提示词。工作流里用 `type: video` 步骤可以直接出片。以下示例成片均由 Agency Orchestrator 实际生成（每条标注了出片方与模型）。")}
          </p>
