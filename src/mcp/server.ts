...
export async function startServer(verbose = false): Promise<void> {
  ...
  server.tool(
    ...
    'Execute a YAML workflow with the DAG engine',
    {
      ...
      provider: z.enum(['claude-code', 'antigravity-cli', 'gemini-cli', 'copilot-cli', 'codex-cli', 'openclaw-cli', 'hermes-cli', 'codebuddy-cli', 'deepseek', 'claude', 'openai', 'ollama']).optional().describe('Override LLM provider'),
